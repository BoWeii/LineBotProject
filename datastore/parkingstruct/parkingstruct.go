...
type ParkingLotNTPC struct {
	...
	TotalCar     string  `json:"totalCar"`      //總汽車數
	TotalMotor   string  `json:"totalMotor"`    //總機車數
	Lat          float64 `json:"twd97X,string"` //緯度
	Lon          float64 `json:"twd97Y,string"` //經度
	AvailableCar int     `json:"availableCar"`  //剩餘數量
}
